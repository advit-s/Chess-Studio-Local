...
def extract_occupied_tiles(metadata_path, limit_records=None):
    X, Y, piece_sets = [], [], []
    records = []
    ...
    for item in records:
        ...
        for index in range(64):
            ...
            X.append(tile_arr)
            Y.append(cls_idx - 1)
            piece_sets.append(item["piece_set"])
    ...
    for case in regression_cases:
        ...
        for index in range(64):
            ...
            if cls_idx > 0:
                ...
                # Oversample occupied pieces 250x, extra oversample for King classes
                repeat_count = 350 if cls_idx in [1, 7] else 250
...
def balance_and_oversample(X, Y, target_per_class=15000):
    X_balanced, Y_balanced = [], []
    for cls in range(12):
        indices = np.where(Y == cls)[0]
        if len(indices) == 0:
            continue
        # Oversample King classes (0 = wk, 6 = bk) even further for 100% King Accuracy
        mult = 1.2 if cls in [0, 6] else 1.0
        target = int(target_per_class * mult)
        selected_indices = np.random.choice(indices, target, replace=True)
...
# ----------------------------------------------------
# PyTorch CNN Architecture (Identical to Keras)
# ----------------------------------------------------
...
def main():
    ...
    X_train_gray, Y_train_gray, _ = extract_occupied_tiles(train_meta, limit_records=40000)
    
    print("Loading validation data (Grayscale)...")
    X_val_gray, Y_val_gray, val_piece_sets = extract_occupied_tiles(val_meta, limit_records=3000)
    ...
    if len(X_reg) > 0:
        ...
        Y_train_gray_b = np.concatenate([Y_train_gray_b, np.array(Y_reg, dtype=np.int64)], axis=0)
    ...
    # Transpose NHWC (B, 32, 32, 1) -> NCHW (B, 1, 32, 32) for PyTorch
    X_train_pt = torch.tensor(X_train_gray_b.transpose(0, 3, 1, 2), dtype=torch.float32)
    Y_train_pt = torch.tensor(Y_train_gray_b, dtype=torch.long)
    
    X_val_pt = torch.tensor(X_val_gray.transpose(0, 3, 1, 2), dtype=torch.float32)
    Y_val_pt = torch.tensor(Y_val_gray, dtype=torch.long)
    ...
    print("\n====================================================")
    print("TRAINING STAGE B: PYTORCH MODEL ON RTX 3050 GPU")
    print("====================================================")
    ...
    for epoch in range(1, 51):
        ...
        print(f"Epoch {epoch:02d}/50 - loss: {train_loss:.4f} - acc: {train_acc:.4f} - val_loss: {val_loss:.4f} - val_acc: {val_acc:.4f} - lr: {lr:.6f}")
        
    print(f"\nBest Validation Loss: {best_val_loss:.4f}")
    model.load_state_dict(best_weights)
    
    # ----------------------------------------------------
    # Convert PyTorch weights to Keras model & save .h5
    # ----------------------------------------------------
    print("\nTransferring GPU weights to Keras model format...")
    ...
    keras_model = models.Sequential([
        ...
        layers.Activation('relu'),
        layers.Dropout(0.4),
        layers.Dense(12, activation='softmax', name='fc2')
    ])
    
    # Helper to convert PyTorch Conv2D (Out, In, H, W) -> Keras Conv2D (H, W, In, Out)
    ...
    # Conv2_2
    keras_model.get_layer('conv2_2').set_weights([pt_to_keras_conv(sd['conv2_2.weight']), sd['conv2_2.bias'].cpu().numpy()])
    keras_model.get_layer('bn2_2').set_weights(pt_to_keras_bn(model.bn2_2))
    
    # Conv3_1
    keras_model.get_layer('conv3_1').set_weights([pt_to_keras_conv(sd['conv3_1.weight']), sd['conv3_1.bias'].cpu().numpy()])
    keras_model.get_layer('bn3_1').set_weights(pt_to_keras_bn(model.bn3_1))
    
    # Conv3_2
    keras_model.get_layer('conv3_2').set_weights([pt_to_keras_conv(sd['conv3_2.weight']), sd['conv3_2.bias'].cpu().numpy()])
    keras_model.get_layer('bn3_2').set_weights(pt_to_keras_bn(model.bn3_2))
    
    # Conv4_1
    keras_model.get_layer('conv4_1').set_weights([pt_to_keras_conv(sd['conv4_1.weight']), sd['conv4_1.bias'].cpu().numpy()])
    keras_model.get_layer('bn4_1').set_weights(pt_to_keras_bn(model.bn4_1))
    ...
    out_model = "data/pieces_model.h5"
    os.makedirs(os.path.dirname(out_model), exist_ok=True)
    keras_model.save(out_model)
    print(f"Successfully saved GPU-trained pieces model to {out_model}")
